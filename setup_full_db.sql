...
-- Migration-safe: enforce cascade deletes across the NOS hierarchy.
-- The live DB still has the original RESTRICT foreign key on units.trade_id,
-- which blocks deleting a trade. Drop and re-add it with ON DELETE CASCADE.
ALTER TABLE public.units
    DROP CONSTRAINT IF EXISTS units_trade_id_fkey;
ALTER TABLE public.units
    ADD CONSTRAINT units_trade_id_fkey
    FOREIGN KEY (trade_id) REFERENCES public.trades(id) ON DELETE CASCADE;

ALTER TABLE public.learning_outcomes
    DROP CONSTRAINT IF EXISTS learning_outcomes_unit_id_fkey;
ALTER TABLE public.learning_outcomes
    ADD CONSTRAINT learning_outcomes_unit_id_fkey
    FOREIGN KEY (unit_id) REFERENCES public.units(id) ON DELETE CASCADE;

ALTER TABLE public.performance_criteria
    DROP CONSTRAINT IF EXISTS performance_criteria_lo_id_fkey;
ALTER TABLE public.performance_criteria
    ADD CONSTRAINT performance_criteria_lo_id_fkey
    FOREIGN KEY (lo_id) REFERENCES public.learning_outcomes(id) ON DELETE CASCADE;
